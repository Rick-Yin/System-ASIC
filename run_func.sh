...
stage_enabled() {
  local stage_id="$1"
  local token
  if [[ -z "${RUN_STAGES// }" ]]; then
    return 0
  fi
  for token in ${RUN_STAGES//,/ }; do
    if [[ "$token" == "$stage_id" ]]; then
      return 0
    fi
  done
  return 1
...
if [[ -n "${RUN_STAGES// }" ]]; then
  echo "[ALL] run_stages=$RUN_STAGES"
fi
if [[ -x "$TIME_BIN" ]]; then
...
if stage_enabled "joint_top"; then
  run_stage "joint_top" "Joint top regression" \
    bash "$ROOT_DIR/vsrc/Joint-CFR-DPD/tb/top/run_top_iverilog.sh" "$RUN_ROOT/joint_top"
else
  echo "[ALL][SKIP] stage=joint_top"
fi

if stage_enabled "joint_l0"; then
  run_stage "joint_l0" "Joint L0 regression" \
    bash "$ROOT_DIR/vsrc/Joint-CFR-DPD/tb/l0_ops/run_l0_iverilog.sh" "$RUN_ROOT/l0"
else
  echo "[ALL][SKIP] stage=joint_l0"
fi

if stage_enabled "migo_tb"; then
  run_stage "migo_tb" "MIGO bit-exact regression" \
    bash "$ROOT_DIR/syn_flow/designs/migo/tb/run_migo_iverilog.sh" "$RUN_ROOT/migo"
else
  echo "[ALL][SKIP] stage=migo_tb"
fi

if stage_enabled "migo_struct"; then
  run_stage "migo_struct" "MIGO structure regression" \
    bash "$ROOT_DIR/syn_flow/designs/migo/tb/run_migo_struct_iverilog.sh" "$RUN_ROOT/migo"
else
  echo "[ALL][SKIP] stage=migo_struct"
fi
