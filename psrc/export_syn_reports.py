...
The summary fields target the synthesis portion of exp.md Table 1:
  - module
  - unit_area
  - total_power_mw
  - wns_ns / whs_ns
...
def fmt_decimal(value: float | None, digits: int = 6) -> str:
    if value is None:
        return ""
...
def infer_clock_ns(config_path: Path, run_root: Path, top_module: str) -> str:
...
def parse_first_number(text: str, patterns: Sequence[str]) -> float | None:
...
def parse_named_power_w(text: str, name: str) -> float | None:
    pattern = re.compile(
...
def parse_total_power_mw(report_path: Path) -> float | None:
...
def parse_worst_slack(report_path: Path) -> float | None:
    if not report_path.exists():
        return None
...
def write_single_row_csv(path: Path, fieldnames: Sequence[str], row: Dict[str, str]) -> None:
...
def upsert_csv(path: Path, fieldnames: Sequence[str], row: Dict[str, str], key_fields: Sequence[str]) -> None:
...
def relpath_text(path: Path, repo_root: Path) -> str:
    try:
...
def export_dc_artifacts(run_root: Path, export_dir: Path) -> None:
...
def export_pt_artifacts(run_root: Path, export_dir: Path) -> None:
    reset_directory(export_dir)
...
def empty_table1_row(module_name: str) -> Dict[str, str]:
    return {
        "模块": module_name,
        "顶层验证": "",
        "结构级验证": "",
...
def build_dc_row(repo_root: Path, run_root: Path, export_dir: Path, design: str, top_module: str, clock_ns: str) -> Dict[str, str]:
    area_report = run_root / "reports" / "area.rpt"
    unit_area = parse_area(area_report)
    row = empty_table1_row(DISPLAY_NAME_MAP.get(design, design))
    row["单元面积"] = fmt_decimal(unit_area)
    return row


def build_pt_row(repo_root: Path, run_root: Path, export_dir: Path, design: str, top_module: str, clock_ns: str) -> Dict[str, str]:
    ...
    row = empty_table1_row(DISPLAY_NAME_MAP.get(design, design))
    row["总功耗"] = fmt_decimal(total_power_mw)
    row["WNS/WHS"] = wns_whs_ns
    return row
